...
Model* Model::CreateFromObj(const std::string& Modelname)
{
	...
	while (getline(file, line)) {
		...
		if (key == "mtllib")
		{
			...
			model->LoadMaterial(directoryPath, filename);
	...
	UINT sizeVB = static_cast<UINT> (sizeof(Model:: VertexPosNormalUv) * model->vertices.size());
	...
	model->vbView.StrideInBytes = sizeof(vertices[0]);
